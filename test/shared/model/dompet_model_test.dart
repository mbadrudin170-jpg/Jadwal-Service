...
void main() {
  group('DompetModel', () {
    ...
    test('01. harus membuat instance DompetModel dengan id yang dibuat secara otomatis', () {
    ...
    test('02. harus membuat instance DompetModel dengan id yang disediakan', () {
      final dompet = DompetModel(id: 'custom-id', name: 'Dompet Custom', balance: 0);
      expect(dompet.id, 'custom-id');
      expect(dompet.name, 'Dompet Custom');
    });

    test('03. harus membuat instance DompetModel dengan nilai default untuk isDeleted dan archivedAt', () {
      final dompet = DompetModel(name: 'Dompet Default', balance: 0);
      expect(dompet.isDeleted, false);
      expect(dompet.archivedAt, isNull);
    });

    test('04. harus menyalin objek dengan semua field tidak berubah jika tidak ada parameter yang diberikan', () {
    ...
    test('09. harus menyalin objek dengan updatedAt yang diperbarui', () {
      ...
      final salinanDompet = dataDompetLengkap.copyWith(updatedAt: tanggalUpdate);
    ...
    test('11. harus menyalin objek dengan archivedAt yang diperbarui', () {
      ...
      final salinanDompet = dataDompetLengkap.copyWith(archivedAt: tanggalArsip);
    ...
    group('fromSqlite', () {
      test('12. harus membuat DompetModel dari peta SQLite dengan semua field', () {
        final mapSqlite = {
          ...
          NamaKolom.name: 'Dompet SQLite',
          NamaKolom.balance: 75000.0,
          NamaKolom.updatedAt: tanggalSekarang.millisecondsSinceEpoch,
          NamaKolom.isDeleted: 1,
          NamaKolom.archivedAt: tanggalSekarang.millisecondsSinceEpoch,
        };
        ...
        expect(dompet.updatedAt, DateTime.fromMillisecondsSinceEpoch(tanggalSekarang.millisecondsSinceEpoch));
        expect(dompet.isDeleted, true);
        expect(dompet.archivedAt, DateTime.fromMillisecondsSinceEpoch(tanggalSekarang.millisecondsSinceEpoch));
      });

      test('13. harus membuat DompetModel dari peta SQLite dengan field yang hilang dan menggunakan nilai default', () {
        final mapSqlite = {
          NamaKolom.id: 'sqlite-2',
          NamaKolom.name: 'Dompet Minimal',
        };
      ...
      test('14. harus menangani nilai updatedAt dan archivedAt sebagai integer', () {
        ...
        final mapSqlite = {
          ...
          NamaKolom.name: 'Dompet Timestamp',
          NamaKolom.updatedAt: timestamp,
          NamaKolom.archivedAt: timestamp,
        };
        final dompet = DompetModel.fromSqlite(mapSqlite);
        expect(dompet.updatedAt, DateTime.fromMillisecondsSinceEpoch(timestamp));
        expect(dompet.archivedAt, DateTime.fromMillisecondsSinceEpoch(timestamp));
      });
    });

    group('toSqlite', () {
      test('15. harus mengubah DompetModel menjadi peta SQLite dengan semua field', () {
        final peta = dataDompetLengkap.toSqlite();

        expect(peta[NamaKolom.id], 'dompet-123');
        expect(peta[NamaKolom.name], 'Dompet Utama');
        expect(peta[NamaKolom.balance], 150000.0);
        expect(peta[NamaKolom.updatedAt], tanggalSekarang.millisecondsSinceEpoch);
        expect(peta[NamaKolom.isDeleted], 0);
        expect(peta[NamaKolom.archivedAt], tanggalSekarang.millisecondsSinceEpoch);
      });

      test('16. harus mengubah DompetModel menjadi peta SQLite dan updatedAt menggunakan DateTime.now() jika null', () {
        final dompetTanpaUpdate = DompetModel(name: 'Baru', balance: 10);
        final peta = dompetTanpaUpdate.toSqlite();

        expect(peta[NamaKolom.updatedAt], isNotNull);
      });

      test('17. harus mengubah isDeleted menjadi 1 jika true dan 0 jika false', () {
        final dompetDihapus = DompetModel(name: 'Hapus', balance: 0, isDeleted: true);
        final dompetTidakDihapus = DompetModel(name: 'Tidak Hapus', balance: 0, isDeleted: false);
        
        expect(dompetDihapus.toSqlite()[NamaKolom.isDeleted], 1);
        expect(dompetTidakDihapus.toSqlite()[NamaKolom.isDeleted], 0);
      });
    });
    ...
    group('fromFirebase', () {
      ...
      test('18. harus membuat DompetModel dari data Firebase dengan semua field', () {
        final dataFirebase = {
          NamaKolom.name: 'Dompet Firebase',
          NamaKolom.balance: 250000.0,
          NamaKolom.updatedAt: timestampFirebase,
          NamaKolom.isDeleted: true,
          NamaKolom.archivedAt: timestampFirebase,
        };
      ...
      test('19. harus membuat DompetModel dari data Firebase dengan field yang hilang dan menggunakan nilai default', () {
        final dataFirebase = {
          NamaKolom.name: 'Dompet Firebase Minimal',
        };
      ...
      test('20. harus menangani nilai updatedAt dan archivedAt sebagai Timestamp', () {
        final dataFirebase = {
          NamaKolom.name: 'Dompet Firebase Timestamp',
          NamaKolom.updatedAt: timestampFirebase,
          NamaKolom.archivedAt: timestampFirebase,
        };
    ...
    group('toFirebase', () {
      test('21. harus mengubah DompetModel menjadi peta Firebase dengan semua field', () {
        ...
        expect(peta[NamaKolom.name], 'Dompet Utama');
        expect(peta[NamaKolom.balance], 150000.0);
        expect(peta[NamaKolom.isDeleted], false);
        expect(peta[NamaKolom.updatedAt], timestampUTC);
        expect(peta[NamaKolom.archivedAt], timestampUTC);
      });

      test('22. harus mengubah DompetModel menjadi peta Firebase dan updatedAt menggunakan DateTime.now() jika null dan dikonversi ke UTC', () {
        final dompetTanpaUpdate = DompetModel(name: 'Baru', balance: 10);
        final peta = dompetTanpaUpdate.toFirebase();

        expect(peta[NamaKolom.updatedAt], isA<Timestamp>());
      });

      test('23. harus mengubah archivedAt menjadi Timestamp UTC jika tidak null', () {
        final dompetDenganArsip = DompetModel(name: 'Arsip', balance: 0, archivedAt: tanggalSekarang);
        final peta = dompetDenganArsip.toFirebase();

        expect(peta[NamaKolom.archivedAt], isA<Timestamp>());
        expect(peta[NamaKolom.archivedAt], Timestamp.fromDate(tanggalSekarang.toUtc()));
      });
    });
  });
}
