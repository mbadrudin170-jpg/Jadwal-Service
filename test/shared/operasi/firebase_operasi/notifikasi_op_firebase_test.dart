...
void main() {
  ...
  group('getNotifAktif', () {
    test(
      ...
      expect(
          ...
          emits(isA<List<NotifikasiModel>>()
              ...
              .having((list) => list.map((e) => e.id), 'id notifikasi',
    ...
    test('05. harus memfilter notifikasi yang tanggal tampilnya di masa depan',
        () async {
      await firestore
          .collection(NamaTabel.notifikasi)
          .doc(notif1.id)
  ...
  group('getById', () {
    ...
    test('11. harus mengembalikan stream kosong jika dokumen tidak ada',
        () async {
      final stream = notifikasiOp.getById('id-tidak-ada');
      expect(stream, emits(isEmpty));
    });

    test('12. harus mengembalikan stream kosong jika notifikasi sudah dihapus',
        () async {
      await firestore
          .collection(NamaTabel.notifikasi)
          .doc(notifDihapus.id)
    ...
    test('13. harus mengembalikan stream kosong jika notifikasi sudah dibaca',
        () async {
      await firestore
          .collection(NamaTabel.notifikasi)
          .doc(notifDibaca.id)
  ...
  group('getKhususAdmin', () {
    ...
    test('16. harus memfilter notifikasi yang bukan tipe order atau transaksi',
        () async {
      await firestore
          .collection(NamaTabel.notifikasi)
          .doc(notifOrder.id)
      ...
      expect(
          ...
          emits(isA<List<NotifikasiModel>>()
              .having((list) => list.length, 'panjang list', 1)
              .having(
                  (list) => list.first.id, 'id notifikasi', notifOrder.id)));
    });
  ...
  group('deleteByTransactionId', () {
    test('25. harus menghapus notifikasi yang cocok dengan transactionId',
        () async {
      ...
      await notifikasiOp.deleteByTransactionId('trx123');
    ...
    test('26. tidak boleh gagal jika tidak ada notifikasi yang cocok',
        () async {
      await firestore
          .collection(NamaTabel.notifikasi)
          .doc(notif1.id)
      ...
      await notifikasiOp.deleteByTransactionId('trx-tidak-ada');
