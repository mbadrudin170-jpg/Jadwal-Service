...
void main() {
  group('PengelolaAkun Notifier', () {
      ...
      container = ProviderContainer(
        overrides: [
          layananPenyimpananLokalProvider
              .overrideWithValue(AsyncData(mockLayananPenyimpananLokal)),
        ],
      );
    });
    ...
    test('02. login should save account and update state', () async {
      // Initial setup
      when(mockLayananPenyimpananLokal.ambilAkunLogin()).thenAnswer((_) async => null);
      when(mockLayananPenyimpananLokal.ambilDaftarAkun())
      ...
      verify(mockLayananPenyimpananLokal.simpanAkunSaatIni(pelanggan1)).called(1);
    ...
    test('03. logout should clear current account and update state', () async {
      ...
      // Prepare for logout
      when(mockLayananPenyimpananLokal.hapusAkunSaatIni()).thenAnswer((_) async => Future.value());
      when(mockLayananPenyimpananLokal.ambilAkunLogin()).thenAnswer((_) async => null);
    ...
    test('04. hapusAkun should remove account from list and update state', () async {
      // Initial setup
      when(mockLayananPenyimpananLokal.ambilAkunLogin())
          .thenAnswer((_) async => pelanggan1);
      when(mockLayananPenyimpananLokal.ambilDaftarAkun())
      ...
      when(mockLayananPenyimpananLokal.hapusAkun('2'))
     ...
     test('05. hapusAkun should also clear current account if it is deleted', () async {
      // Initial setup
      when(mockLayananPenyimpananLokal.ambilAkunLogin())
          .thenAnswer((_) async => pelanggan1);
      when(mockLayananPenyimpananLokal.ambilDaftarAkun())
      ...
      // Prepare for deletion
      when(mockLayananPenyimpananLokal.hapusAkun('1'))
          .thenAnswer((_) async => Future.value());
