...
void main() {
  ...
  final kategoriPemasukan =
      KategoriModel(id: 'kp1', nama: 'Gaji', tipe: TipeKategori.income);
  final kategoriPengeluaran =
      KategoriModel(id: 'kl1', nama: 'Makan', tipe: TipeKategori.expense);
  ...
  setUp(() {
    ...
    when(() => mockSinkronisasiService.jalankanCekSinkronisasi())
        .thenAnswer((_) async {});

    registerFallbackValue(TransaksiModel(
        id: '',
        deskripsi: '',
        jumlah: 0,
        tanggal: DateTime.now(),
    ...
    return ProviderScope(
      overrides: [
        ...
        layananCekSinkronisasiProvider
            .overrideWithValue(mockSinkronisasiService),
      ],
      child: MaterialApp(
  ...
  group('Inisialisasi & Rendering', () {
    testWidgets(
        '01. Mode Tambah: Harus merender judul "Tambah Transaksi" dan form kosong',
        (tester) async {
    ...
    testWidgets(
        '02. Mode Edit: Harus merender judul "Edit Transaksi" dan mengisi form',
        (tester) async {
    ...
    testWidgets(
        '03. Harus menampilkan CircularProgressIndicator saat loading data',
        (tester) async {
    ...
    testWidgets('04. Harus menampilkan pesan error jika gagal memuat data awal',
        (tester) async {
      when(() => mockDompetOp.ambilSemua())
  ...
  group('Interaksi Form & Validasi', () {
    testWidgets(
        '06. Harus menampilkan pesan error validasi jika field wajib kosong',
        (tester) async {
      await tester.pumpWidget(createWidgetUnderTest());
      await tester.pumpAndSettle();
    ...
    testWidgets(
        '07. Harus menampilkan dropdown Dompet Tujuan saat tipe Transfer dipilih',
        (tester) async {
      await tester.pumpWidget(createWidgetUnderTest());
      await tester.pumpAndSettle();
    ...
    testWidgets(
        '08. Transfer: Harus error jika dompet tujuan sama dengan dompet asal',
        (tester) async {
      await tester.pumpWidget(createWidgetUnderTest());
      await tester.pumpAndSettle();
      ...
      await tester.tap(find.text('Pilih Dompet Tujuan'));
      await tester.pumpAndSettle();
      await tester.tap(find.text('Dompet A').last);
      await tester.pumpAndSettle();
    ...
    testWidgets('09. Harus memfilter kategori saat tipe transaksi diubah',
        (tester) async {
      await tester.pumpWidget(createWidgetUnderTest());
      await tester.pumpAndSettle();
  ...
  group('Proses Penyimpanan', () {
    testWidgets(
        '11. Mode Tambah: Harus berhasil simpan & sinkronisasi saat online',
        (tester) async {
      await tester.pumpWidget(createWidgetUnderTest());
      await tester.pumpAndSettle();
      ...
      expect(find.text('Transaksi berhasil disimpan dan disinkronkan.'),
          findsOneWidget);
      verify(() => mockNavigatorObserver.didPop(any(), any())).called(1);
    });

    testWidgets('12. Mode Tambah: Harus berhasil simpan lokal saat offline',
        (tester) async {
      when(() => mockKoneksiService.cekKoneksiLokal())
    ...
    testWidgets('13. Mode Edit: Harus berhasil memperbarui transaksi',
        (tester) async {
      await tester.pumpWidget(createWidgetUnderTest(transaksi: mockTransaksi));
      await tester.pumpAndSettle();
      ...
      verify(() => mockTransaksiOp.updateTransaction(
          mockTransaksi.id,
          any(
              that: isA<TransaksiModel>()
                ..having((t) => t.deskripsi, 'deskripsi', 'Makan Malam'))));
    });

    testWidgets('14. Harus menampilkan toast error jika penyimpanan gagal',
        (tester) async {
      when(() => mockTransaksiOp.tambahTransaksi(any()))
