...
void main() {
  ...
  setUp(() {
    ...
    // Perbaikan: gunakan anyNamed untuk argumen bernama
    when(mockStatusUnggahOpSqlite.tandaiButuhUpload(true,
  ...
  group('BaseOpSqlite', () {
    ...
    group('sisipkan', () {
      test(
        ...
        () async {
          ...
          when(mockTxn.insert(any, any, conflictAlgorithm: any))
          ...
          verify(mockStatusUnggahOpSqlite.tandaiButuhUpload(true,
                  transaction: mockTxn))
              .called(1);
        },
      );
      ...
      test('02. harus melempar kembali exception jika transaksi gagal', () {
        ...
        expect(
            baseOpSqlite.sisipkan(namaTabel, data), throwsA(isA<Exception>()));
      });
    });
    ...
    group('update', () {
      test(
        ...
        () async {
          ...
          when(mockTxn.update(any, any, where: any, whereArgs: any))
              .thenAnswer((_) async => 1);
    ...
    group('delete', () {
      test('05. harus memanggil db.delete dengan klausa where yang benar',
          () async {
        final mockTxn = MockTransaction();
        when(mockDatabase.transaction<int>(any)).thenAnswer((invocation) async {
          final action = invocation.positionalArguments[0] as Future<int>
              Function(Transaction);
          return await action(mockTxn);
        });
        when(mockTxn.delete(any, where: any, whereArgs: any))
            .thenAnswer((_) async => 1);
    ...
    group('softDelete', () {
      test(
        ...
        verify(mockTxn.update(
          ...
          argThat(isA<Map<String, dynamic>>()
              .having((map) => map[NamaKolom.dihapus], 'dihapus', 1)),
          where: '${NamaKolom.id} = ?',
          whereArgs: ['1'],
        )).called(1);
      });
      ...
      test('08. harus melempar kembali exception jika transaksi gagal', () {
        ...
        expect(
            baseOpSqlite.softDelete(namaTabel, '1'), throwsA(isA<Exception>()));
      });
    });
    ...
    group('sisipkanAtauPerbaruiBatch', () {
      ...
      test('09. harus menjalankan operasi batch insert di dalam transaksi',
        ...
        when(mockBatch.insert(any, any, conflictAlgorithm: any))
            .thenAnswer((_) {});
    ...
    group('runComplexOperation', () {
      ...
      test(
        ...
        expect(baseOpSqlite.runComplexOperation<void>((txn) async {}),
            throwsA(isA<Exception>()));
      });
    });
  });
