...
class _StatistikPageAState extends ConsumerState<StatistikPageA> {
  ...
  final List<FlSpot> _weeklySpots = [
    const FlSpot(0, 1.2), const FlSpot(1, 1.5), const FlSpot(2, 1.1),
    const FlSpot(3, 1.8),
  ];
  final List<FlSpot> _dailySpots = [
    const FlSpot(0, 0.2), const FlSpot(1, 0.5), const FlSpot(2, 0.4),
    const FlSpot(3, 0.8), const FlSpot(4, 0.6), const FlSpot(5, 1.1),
    const FlSpot(6, 1.0),
  ];
  ...
  double get _maxX => switch (_selectedRange) {
    ChartRange.harian => 6, ChartRange.mingguan => 3, ChartRange.bulanan => 4,
  };

  double get _maxY => switch (_selectedRange) {
    ChartRange.harian => 2, ChartRange.mingguan => 3, ChartRange.bulanan => 6,
  };
    ...
    return Scaffold(
      ...
      body: RefreshIndicator(
        ...
        child: statistikStateAsync.when(
          ...
          error: (err, stack) => Center(child: Text('Error: ${err.toString()}')), // Penanganan error sederhana
          data: (data) {
                ...
                children: [
                  ...
                  Wrap(
                    ...
                    children: [
                      ...
                      _buildStatCardWrapper(
                        onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => const ActiveCustomerPage())),
                      ...
                      _buildStatCardWrapper(
                        onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => const TransactionPage())),
                        title: 'Pendapatan Bulan Ini',
                        value: CurrencyFormat.formatCurrency(data.pendapatanBulanIni),
                        icon: TIcons.money,
                        color: data.pendapatanBulanIni < 0 ? Colors.red : Colors.orange,
                      ),
                       _buildStatCardWrapper(
  ...
  Widget _buildStatCardWrapper({
    ...
    return GestureDetector(
      onTap: onTap,
      child: _buildStatCard(title: title, value: value, icon: icon, color: color),
    );
  }
    ...
    return Row(
      ...
      children: [
        ToggleButtons(
          ...
          onPressed: (index) => setState(() => _selectedRange = ChartRange.values[index]),
  ...
  Widget _buildBestSellingPackages(ThemeData theme, List<BestSellingPackage> packages) {
    if (packages.isEmpty) {
      return const Card(
        elevation: 2,
      ...
      child: Column(
        children: List.generate(packages.length, (index) {
          ...
          return ListTile(
            ...
            trailing: Text('${item.totalSold} terjual', style: theme.textTheme.bodyMedium?.copyWith(fontWeight: FontWeight.bold)),
          );
        }),
      ),
    );
  ...
  LineChartData _mainLineChartData() {
     return LineChartData(
      ...
      titlesData: FlTitlesData(
        ...
        leftTitles: AxisTitles(sideTitles: SideTitles(showTitles: true, interval: 1, getTitlesWidget: _leftTitleWidgets, reservedSize: 42)),
      ),
      borderData: FlBorderData(show: true, border: Border.all(color: const Color(0xff37434d))),
      minX: 0, maxX: _maxX, minY: 0, maxY: _maxY,
  ...
  LineChartBarData _mainLineBarData() {
    return LineChartBarData(
      ...
      belowBarData: BarAreaData(show: true, gradient: LinearGradient(colors: [Theme.of(context).colorScheme.primary.withAlpha(20), Theme.of(context).colorScheme.primary.withAlpha(50)])),
    );
  }
  ...
  Widget _bottomTitleWidgets(double value, TitleMeta meta) {
    ...
    switch (_selectedRange) {
      case ChartRange.harian:
        text = switch (value.toInt()) { 0 => const Text('Sen', style: style), 1 => const Text('Sel', style: style), 2 => const Text('Rab', style: style), 3 => const Text('Kam', style: style), 4 => const Text('Jum', style: style), 5 => const Text('Sab', style: style), 6 => const Text('Min', style: style), _ => const Text('', style: style) };
        break;
      case ChartRange.mingguan:
        text = switch (value.toInt()) { 0 => const Text('M1', style: style), 1 => const Text('M2', style: style), 2 => const Text('M3', style: style), 3 => const Text('M4', style: style), _ => const Text('', style: style) };
        break;
      case ChartRange.bulanan:
        text = switch (value.toInt()) { 0 => const Text('JAN', style: style), 1 => const Text('FEB', style: style), 2 => const Text('MAR', style: style), 3 => const Text('APR', style: style), 4 => const Text('MEI', style: style), _ => const Text('', style: style) };
        break;
    }
  ...
  Widget _leftTitleWidgets(double value, TitleMeta meta) {
    ...
    switch (_selectedRange) {
      ...
      case ChartRange.mingguan:
        ...
        break;
      case ChartRange.bulanan:
        text = switch (value.toInt()) { 1 => '1JT', 3 => '3JT', 5 => '5JT', _ => '' };
        break;
    }
