// GENERATED CODE - DO NOT MODIFY BY HAND

part of 'operasi_sqlite_provider.dart';

// **************************************************************************
// RiverpodGenerator
// **************************************************************************

// GENERATED CODE - DO NOT MODIFY BY HAND
// ignore_for_file: type=lint, type=warning
/// Provider untuk menyediakan instance dari [PackageOperation].

@ProviderFor(packageOperation)
final packageOperationProvider = PackageOperationProvider._();

/// Provider untuk menyediakan instance dari [PackageOperation].

final class PackageOperationProvider extends $FunctionalProvider<
    PackageOperation,
    PackageOperation,
    PackageOperation> with $Provider<PackageOperation> {
  /// Provider untuk menyediakan instance dari [PackageOperation].
  PackageOperationProvider._()
      : super(
          from: null,
          argument: null,
          retry: null,
          name: r'packageOperationProvider',
          isAutoDispose: false,
          dependencies: null,
          $allTransitiveDependencies: null,
        );

  @override
  String debugGetCreateSourceHash() => _$packageOperationHash();

  @$internal
  @override
  $ProviderElement<PackageOperation> $createElement($ProviderPointer pointer) =>
      $ProviderElement(pointer);

  @override
  PackageOperation create(Ref ref) {
    return packageOperation(ref);
  }

  /// {@macro riverpod.override_with_value}
  Override overrideWithValue(PackageOperation value) {
    return $ProviderOverride(
      origin: this,
      providerOverride: $SyncValueProvider<PackageOperation>(value),
    );
  }
}

String _$packageOperationHash() => r'd8bf4fb8eccee62dea626cb535bd390ff4e0960a';

/// Provider untuk menyediakan instance dari [TransactionOperation].

@ProviderFor(transactionOperation)
final transactionOperationProvider = TransactionOperationProvider._();

/// Provider untuk menyediakan instance dari [TransactionOperation].

final class TransactionOperationProvider extends $FunctionalProvider<
    TransactionOperation,
    TransactionOperation,
    TransactionOperation> with $Provider<TransactionOperation> {
  /// Provider untuk menyediakan instance dari [TransactionOperation].
  TransactionOperationProvider._()
      : super(
          from: null,
          argument: null,
          retry: null,
          name: r'transactionOperationProvider',
          isAutoDispose: false,
          dependencies: null,
          $allTransitiveDependencies: null,
        );

  @override
  String debugGetCreateSourceHash() => _$transactionOperationHash();

  @$internal
  @override
  $ProviderElement<TransactionOperation> $createElement(
          $ProviderPointer pointer) =>
      $ProviderElement(pointer);

  @override
  TransactionOperation create(Ref ref) {
    return transactionOperation(ref);
  }

  /// {@macro riverpod.override_with_value}
  Override overrideWithValue(TransactionOperation value) {
    return $ProviderOverride(
      origin: this,
      providerOverride: $SyncValueProvider<TransactionOperation>(value),
    );
  }
}

String _$transactionOperationHash() =>
    r'314c5f3ce4808cf1df52fde215fba4192003e30f';

/// Provider untuk menyediakan instance dari [CustomerOperation].

@ProviderFor(customerOperation)
final customerOperationProvider = CustomerOperationProvider._();

/// Provider untuk menyediakan instance dari [CustomerOperation].

final class CustomerOperationProvider extends $FunctionalProvider<
    CustomerOperation,
    CustomerOperation,
    CustomerOperation> with $Provider<CustomerOperation> {
  /// Provider untuk menyediakan instance dari [CustomerOperation].
  CustomerOperationProvider._()
      : super(
          from: null,
          argument: null,
          retry: null,
          name: r'customerOperationProvider',
          isAutoDispose: false,
          dependencies: null,
          $allTransitiveDependencies: null,
        );

  @override
  String debugGetCreateSourceHash() => _$customerOperationHash();

  @$internal
  @override
  $ProviderElement<CustomerOperation> $createElement(
          $ProviderPointer pointer) =>
      $ProviderElement(pointer);

  @override
  CustomerOperation create(Ref ref) {
    return customerOperation(ref);
  }

  /// {@macro riverpod.override_with_value}
  Override overrideWithValue(CustomerOperation value) {
    return $ProviderOverride(
      origin: this,
      providerOverride: $SyncValueProvider<CustomerOperation>(value),
    );
  }
}

String _$customerOperationHash() => r'41272e4b2b15a81a1bc6741805522d2f0eb396c9';

/// Provider untuk menyediakan instance dari [ActiveCustomerOperation].

@ProviderFor(activeCustomerOperation)
final activeCustomerOperationProvider = ActiveCustomerOperationProvider._();

/// Provider untuk menyediakan instance dari [ActiveCustomerOperation].

final class ActiveCustomerOperationProvider extends $FunctionalProvider<
    ActiveCustomerOperation,
    ActiveCustomerOperation,
    ActiveCustomerOperation> with $Provider<ActiveCustomerOperation> {
  /// Provider untuk menyediakan instance dari [ActiveCustomerOperation].
  ActiveCustomerOperationProvider._()
      : super(
          from: null,
          argument: null,
          retry: null,
          name: r'activeCustomerOperationProvider',
          isAutoDispose: false,
          dependencies: null,
          $allTransitiveDependencies: null,
        );

  @override
  String debugGetCreateSourceHash() => _$activeCustomerOperationHash();

  @$internal
  @override
  $ProviderElement<ActiveCustomerOperation> $createElement(
          $ProviderPointer pointer) =>
      $ProviderElement(pointer);

  @override
  ActiveCustomerOperation create(Ref ref) {
    return activeCustomerOperation(ref);
  }

  /// {@macro riverpod.override_with_value}
  Override overrideWithValue(ActiveCustomerOperation value) {
    return $ProviderOverride(
      origin: this,
      providerOverride: $SyncValueProvider<ActiveCustomerOperation>(value),
    );
  }
}

String _$activeCustomerOperationHash() =>
    r'4e55cd25bda321a7ad5431ed9337f86306a0da0f';

/// Provider untuk menyediakan instance dari [ApkVersionOperation].

@ProviderFor(apkVersionOperation)
final apkVersionOperationProvider = ApkVersionOperationProvider._();

/// Provider untuk menyediakan instance dari [ApkVersionOperation].

final class ApkVersionOperationProvider extends $FunctionalProvider<
    ApkVersionOperation,
    ApkVersionOperation,
    ApkVersionOperation> with $Provider<ApkVersionOperation> {
  /// Provider untuk menyediakan instance dari [ApkVersionOperation].
  ApkVersionOperationProvider._()
      : super(
          from: null,
          argument: null,
          retry: null,
          name: r'apkVersionOperationProvider',
          isAutoDispose: false,
          dependencies: null,
          $allTransitiveDependencies: null,
        );

  @override
  String debugGetCreateSourceHash() => _$apkVersionOperationHash();

  @$internal
  @override
  $ProviderElement<ApkVersionOperation> $createElement(
          $ProviderPointer pointer) =>
      $ProviderElement(pointer);

  @override
  ApkVersionOperation create(Ref ref) {
    return apkVersionOperation(ref);
  }

  /// {@macro riverpod.override_with_value}
  Override overrideWithValue(ApkVersionOperation value) {
    return $ProviderOverride(
      origin: this,
      providerOverride: $SyncValueProvider<ApkVersionOperation>(value),
    );
  }
}

String _$apkVersionOperationHash() =>
    r'2bb15324e8827988428e02d2bd511f5249867141';

/// Provider untuk menyediakan instance dari [CategoryOperation].

@ProviderFor(categoryOperation)
final categoryOperationProvider = CategoryOperationProvider._();

/// Provider untuk menyediakan instance dari [CategoryOperation].

final class CategoryOperationProvider extends $FunctionalProvider<
    CategoryOperation,
    CategoryOperation,
    CategoryOperation> with $Provider<CategoryOperation> {
  /// Provider untuk menyediakan instance dari [CategoryOperation].
  CategoryOperationProvider._()
      : super(
          from: null,
          argument: null,
          retry: null,
          name: r'categoryOperationProvider',
          isAutoDispose: false,
          dependencies: null,
          $allTransitiveDependencies: null,
        );

  @override
  String debugGetCreateSourceHash() => _$categoryOperationHash();

  @$internal
  @override
  $ProviderElement<CategoryOperation> $createElement(
          $ProviderPointer pointer) =>
      $ProviderElement(pointer);

  @override
  CategoryOperation create(Ref ref) {
    return categoryOperation(ref);
  }

  /// {@macro riverpod.override_with_value}
  Override overrideWithValue(CategoryOperation value) {
    return $ProviderOverride(
      origin: this,
      providerOverride: $SyncValueProvider<CategoryOperation>(value),
    );
  }
}

String _$categoryOperationHash() => r'6d08c18cf743efa0effb7d19935c12b456752816';

/// Provider untuk menyediakan instance dari [DataCleaningOperation].

@ProviderFor(dataCleaningOperation)
final dataCleaningOperationProvider = DataCleaningOperationProvider._();

/// Provider untuk menyediakan instance dari [DataCleaningOperation].

final class DataCleaningOperationProvider extends $FunctionalProvider<
    DataCleaningOperation,
    DataCleaningOperation,
    DataCleaningOperation> with $Provider<DataCleaningOperation> {
  /// Provider untuk menyediakan instance dari [DataCleaningOperation].
  DataCleaningOperationProvider._()
      : super(
          from: null,
          argument: null,
          retry: null,
          name: r'dataCleaningOperationProvider',
          isAutoDispose: false,
          dependencies: null,
          $allTransitiveDependencies: null,
        );

  @override
  String debugGetCreateSourceHash() => _$dataCleaningOperationHash();

  @$internal
  @override
  $ProviderElement<DataCleaningOperation> $createElement(
          $ProviderPointer pointer) =>
      $ProviderElement(pointer);

  @override
  DataCleaningOperation create(Ref ref) {
    return dataCleaningOperation(ref);
  }

  /// {@macro riverpod.override_with_value}
  Override overrideWithValue(DataCleaningOperation value) {
    return $ProviderOverride(
      origin: this,
      providerOverride: $SyncValueProvider<DataCleaningOperation>(value),
    );
  }
}

String _$dataCleaningOperationHash() =>
    r'9ee8450a097ed913cf6923d563a2ebfe0442c784';

/// Provider untuk menyediakan instance dari [FeedbackOperation].

@ProviderFor(feedbackOperation)
final feedbackOperationProvider = FeedbackOperationProvider._();

/// Provider untuk menyediakan instance dari [FeedbackOperation].

final class FeedbackOperationProvider extends $FunctionalProvider<
    FeedbackOperation,
    FeedbackOperation,
    FeedbackOperation> with $Provider<FeedbackOperation> {
  /// Provider untuk menyediakan instance dari [FeedbackOperation].
  FeedbackOperationProvider._()
      : super(
          from: null,
          argument: null,
          retry: null,
          name: r'feedbackOperationProvider',
          isAutoDispose: false,
          dependencies: null,
          $allTransitiveDependencies: null,
        );

  @override
  String debugGetCreateSourceHash() => _$feedbackOperationHash();

  @$internal
  @override
  $ProviderElement<FeedbackOperation> $createElement(
          $ProviderPointer pointer) =>
      $ProviderElement(pointer);

  @override
  FeedbackOperation create(Ref ref) {
    return feedbackOperation(ref);
  }

  /// {@macro riverpod.override_with_value}
  Override overrideWithValue(FeedbackOperation value) {
    return $ProviderOverride(
      origin: this,
      providerOverride: $SyncValueProvider<FeedbackOperation>(value),
    );
  }
}

String _$feedbackOperationHash() => r'76f60efc9b539182b588a10811fbff244a50a7f1';

/// Provider untuk menyediakan instance dari [OrderOperation].

@ProviderFor(orderOperation)
final orderOperationProvider = OrderOperationProvider._();

/// Provider untuk menyediakan instance dari [OrderOperation].

final class OrderOperationProvider
    extends $FunctionalProvider<OrderOperation, OrderOperation, OrderOperation>
    with $Provider<OrderOperation> {
  /// Provider untuk menyediakan instance dari [OrderOperation].
  OrderOperationProvider._()
      : super(
          from: null,
          argument: null,
          retry: null,
          name: r'orderOperationProvider',
          isAutoDispose: false,
          dependencies: null,
          $allTransitiveDependencies: null,
        );

  @override
  String debugGetCreateSourceHash() => _$orderOperationHash();

  @$internal
  @override
  $ProviderElement<OrderOperation> $createElement($ProviderPointer pointer) =>
      $ProviderElement(pointer);

  @override
  OrderOperation create(Ref ref) {
    return orderOperation(ref);
  }

  /// {@macro riverpod.override_with_value}
  Override overrideWithValue(OrderOperation value) {
    return $ProviderOverride(
      origin: this,
      providerOverride: $SyncValueProvider<OrderOperation>(value),
    );
  }
}

String _$orderOperationHash() => r'5b1230184032961008baa3350af4ce87c7a8af18';

/// Provider untuk menyediakan instance dari [SettingsOperation].

@ProviderFor(settingsOperation)
final settingsOperationProvider = SettingsOperationProvider._();

/// Provider untuk menyediakan instance dari [SettingsOperation].

final class SettingsOperationProvider extends $FunctionalProvider<
    SettingsOperation,
    SettingsOperation,
    SettingsOperation> with $Provider<SettingsOperation> {
  /// Provider untuk menyediakan instance dari [SettingsOperation].
  SettingsOperationProvider._()
      : super(
          from: null,
          argument: null,
          retry: null,
          name: r'settingsOperationProvider',
          isAutoDispose: false,
          dependencies: null,
          $allTransitiveDependencies: null,
        );

  @override
  String debugGetCreateSourceHash() => _$settingsOperationHash();

  @$internal
  @override
  $ProviderElement<SettingsOperation> $createElement(
          $ProviderPointer pointer) =>
      $ProviderElement(pointer);

  @override
  SettingsOperation create(Ref ref) {
    return settingsOperation(ref);
  }

  /// {@macro riverpod.override_with_value}
  Override overrideWithValue(SettingsOperation value) {
    return $ProviderOverride(
      origin: this,
      providerOverride: $SyncValueProvider<SettingsOperation>(value),
    );
  }
}

String _$settingsOperationHash() => r'957637cda34d493db5ca2e640a214eeb59be7a28';

/// Provider untuk menyediakan instance dari [SubCategoryOperation].

@ProviderFor(subCategoryOperation)
final subCategoryOperationProvider = SubCategoryOperationProvider._();

/// Provider untuk menyediakan instance dari [SubCategoryOperation].

final class SubCategoryOperationProvider extends $FunctionalProvider<
    SubCategoryOperation,
    SubCategoryOperation,
    SubCategoryOperation> with $Provider<SubCategoryOperation> {
  /// Provider untuk menyediakan instance dari [SubCategoryOperation].
  SubCategoryOperationProvider._()
      : super(
          from: null,
          argument: null,
          retry: null,
          name: r'subCategoryOperationProvider',
          isAutoDispose: false,
          dependencies: null,
          $allTransitiveDependencies: null,
        );

  @override
  String debugGetCreateSourceHash() => _$subCategoryOperationHash();

  @$internal
  @override
  $ProviderElement<SubCategoryOperation> $createElement(
          $ProviderPointer pointer) =>
      $ProviderElement(pointer);

  @override
  SubCategoryOperation create(Ref ref) {
    return subCategoryOperation(ref);
  }

  /// {@macro riverpod.override_with_value}
  Override overrideWithValue(SubCategoryOperation value) {
    return $ProviderOverride(
      origin: this,
      providerOverride: $SyncValueProvider<SubCategoryOperation>(value),
    );
  }
}

String _$subCategoryOperationHash() =>
    r'34ed47c602f1b5dd6be378d74f6d7941302046cd';

/// Provider untuk menyediakan instance dari [WalletOperation].

@ProviderFor(walletOperation)
final walletOperationProvider = WalletOperationProvider._();

/// Provider untuk menyediakan instance dari [WalletOperation].

final class WalletOperationProvider extends $FunctionalProvider<WalletOperation,
    WalletOperation, WalletOperation> with $Provider<WalletOperation> {
  /// Provider untuk menyediakan instance dari [WalletOperation].
  WalletOperationProvider._()
      : super(
          from: null,
          argument: null,
          retry: null,
          name: r'walletOperationProvider',
          isAutoDispose: false,
          dependencies: null,
          $allTransitiveDependencies: null,
        );

  @override
  String debugGetCreateSourceHash() => _$walletOperationHash();

  @$internal
  @override
  $ProviderElement<WalletOperation> $createElement($ProviderPointer pointer) =>
      $ProviderElement(pointer);

  @override
  WalletOperation create(Ref ref) {
    return walletOperation(ref);
  }

  /// {@macro riverpod.override_with_value}
  Override overrideWithValue(WalletOperation value) {
    return $ProviderOverride(
      origin: this,
      providerOverride: $SyncValueProvider<WalletOperation>(value),
    );
  }
}

String _$walletOperationHash() => r'21a9dd79824e4c3f4dec5558d5a363162cccf1b5';
